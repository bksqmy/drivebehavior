...
*/
public class SpeedBehavior_low extends BaseBehavior{
	/** 
	 平均速度
	...
	*/
	...
	public void getBehavior(TripBehaviorData bhvdata){
		...
		this.time = speedgroup[2].getTime();
		this.distance = speedgroup[2].getDistance();
		if (num > 0){
		...
		float num2 = (float)(((double)this.average) / 30.0);
		num2 = (num2 < 1f) ? num2 : 1f;
		num2 = (num2 > 0f) ? num2 : 0f;
		super.setEnvmark((int)(this.timescale * (90f + (5f * num2))));
		super.setSafemark((int)(this.timescale * (98f - (2f * num2))));
